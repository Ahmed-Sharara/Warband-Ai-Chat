...
# --- COMPANION MAP TRAVEL ---
# --- COMPANION MAP TRAVEL ---
(0.5, [
    (try_for_range, ":companion", companions_begin, companions_end),
        (neg|main_party_has_troop, ":companion"),
        (troop_slot_eq, ":companion", slot_troop_cur_center, -1),
        
        (assign, ":companion_party", -1),
        (try_for_parties, ":party_no"),
            (party_is_active, ":party_no"),
            (party_count_members_of_type, ":count", ":party_no", ":companion"),
            (gt, ":count", 0),
            (assign, ":companion_party", ":party_no"),
        (try_end),
        
        (try_begin),
            (gt, ":companion_party", 0),
            (try_begin),
                (troop_slot_eq, ":companion", 180, 2), # Returning state
                (store_distance_to_party_from_party, ":dist_to_player", ":companion_party", "p_main_party"),
                (le, ":dist_to_player", 1), # Reached player party
                (party_set_ai_behavior, ":companion_party", ai_bhvr_hold),
                # (change_screen_map_conversation, ":companion"),
            (try_end),
            (party_get_attached_to, ":attached", ":companion_party"),
            (try_begin),
                (is_between, ":attached", centers_begin, centers_end),
                (assign, ":reached_center", ":attached"),
                (try_begin),
                    (troop_get_slot, ":num_tasks", ":companion", 1000),
                    (gt, ":num_tasks", 0),
                    
                    # Advance tasks queue (shift down by 2 slots)
                    (troop_get_slot, ":t2_type", ":companion", 1003),
                    (troop_get_slot, ":t2_arg", ":companion", 1004),
                    (troop_set_slot, ":companion", 1001, ":t2_type"),
                    (troop_set_slot, ":companion", 1002, ":t2_arg"),
                    
                    (troop_get_slot, ":t3_type", ":companion", 1005),
                    (troop_get_slot, ":t3_arg", ":companion", 1006),
                    (troop_set_slot, ":companion", 1003, ":t3_type"),
                    (troop_set_slot, ":companion", 1004, ":t3_arg"),

                    (troop_get_slot, ":t4_type", ":companion", 1007),
                    (troop_get_slot, ":t4_arg", ":companion", 1008),
                    (troop_set_slot, ":companion", 1005, ":t4_type"),
                    (troop_set_slot, ":companion", 1006, ":t4_arg"),

                    (troop_get_slot, ":t5_type", ":companion", 1009),
                    (troop_get_slot, ":t5_arg", ":companion", 1010),
                    (troop_set_slot, ":companion", 1007, ":t5_type"),
                    (troop_set_slot, ":companion", 1008, ":t5_arg"),

                    (troop_set_slot, ":companion", 1009, 0),
                    (troop_set_slot, ":companion", 1010, 0),

                    (val_sub, ":num_tasks", 1),
                    (troop_set_slot, ":companion", 1000, ":num_tasks"),

                    # Process next task
                    (try_begin),
                        (gt, ":num_tasks", 0),
                        (troop_get_slot, ":cur_task_type", ":companion", 1001),
                        (troop_get_slot, ":cur_task_arg", ":companion", 1002),
                        (try_begin),
                            (eq, ":cur_task_type", 1), # Go to town
                            (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                            (party_set_ai_object, ":companion_party", ":cur_task_arg"),
                            (party_set_slot, ":companion_party", slot_party_ai_object, ":cur_task_arg"),
                        (else_try),
                            (eq, ":cur_task_type", 2), # Buy/Fetch item
                            (troop_set_slot, ":companion", 181, ":cur_task_arg"),
                            (troop_set_slot, ":companion", 180, 2), # Returning state

                            # Shift down immediately because purchase is continuous
                            (troop_get_slot, ":t2_type_s", ":companion", 1003),
                            (troop_get_slot, ":t2_arg_s", ":companion", 1004),
                            (troop_set_slot, ":companion", 1001, ":t2_type_s"),
                            (troop_set_slot, ":companion", 1002, ":t2_arg_s"),
                            
                            (troop_get_slot, ":t3_type_s", ":companion", 1005),
                            (troop_get_slot, ":t3_arg_s", ":companion", 1006),
                            (troop_set_slot, ":companion", 1003, ":t3_type_s"),
                            (troop_set_slot, ":companion", 1004, ":t3_arg_s"),

                            (troop_get_slot, ":t4_type_s", ":companion", 1007),
                            (troop_get_slot, ":t4_arg_s", ":companion", 1008),
                            (troop_set_slot, ":companion", 1005, ":t4_type_s"),
                            (troop_set_slot, ":companion", 1006, ":t4_arg_s"),

                            (troop_set_slot, ":companion", 1007, 0),
                            (troop_set_slot, ":companion", 1008, 0),

                            (val_sub, ":num_tasks", 1),
                            (troop_set_slot, ":companion", 1000, ":num_tasks"),

                            # Proceed to next or default back
                            (try_begin),
                                (gt, ":num_tasks", 0),
                                (troop_get_slot, ":next_task_type", ":companion", 1001),
                                (troop_get_slot, ":next_task_arg", ":companion", 1002),
                                (try_begin),
                                    (eq, ":next_task_type", 1),
                                    (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                                    (party_set_ai_object, ":companion_party", ":next_task_arg"),
                                    (party_set_slot, ":companion_party", slot_party_ai_object, ":next_task_arg"),
                                (else_try),
                                    # Default return to main party
                                    (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                                    (party_set_ai_object, ":companion_party", "p_main_party"),
                                    (party_set_slot, ":companion_party", slot_party_ai_object, "p_main_party"),
                                (try_end),
                            (else_try),
                                (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                                (party_set_ai_object, ":companion_party", "p_main_party"),
                            ...
                            (try_end),
                        (else_try),
                            (eq, ":cur_task_type", 3), # Return to player
                            (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                            (party_set_ai_object, ":companion_party", "p_main_party"),
                            (party_set_slot, ":companion_party", slot_party_ai_object, "p_main_party"),
                        (try_end),
                    (else_try),
                        # No tasks left, return to player
                        (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                        (party_set_ai_object, ":companion_party", "p_main_party"),
                        (party_set_slot, ":companion_party", slot_party_ai_object, "p_main_party"),
                    (try_end),
                (else_try),
                    (troop_slot_eq, ":companion", 180, 1), # Fetching state
                    (troop_set_slot, ":companion", 180, 2), # Returning state
                    (party_set_ai_behavior, ":companion_party", ai_bhvr_travel_to_party),
                    (party_set_ai_object, ":companion_party", "p_main_party"),
                    (party_set_slot, ":companion_party", slot_party_ai_object, "p_main_party"),
                (else_try),
                    (troop_set_slot, ":companion", slot_troop_cur_center, ":reached_center"),
                    (troop_set_slot, ":companion", slot_troop_playerparty_history, pp_history_dismissed),
                    (remove_party, ":companion_party"),
                (try_end),
            (else_try),
                (party_get_slot, ":ai_obj", ":companion_party", slot_party_ai_object),
                (is_between, ":ai_obj", centers_begin, centers_end),
                (store_distance_to_party_from_party, ":dist", ":companion_party", ":ai_obj"),
                (le, ":dist", 2),
                (assign, ":reached_center", ":ai_obj"),
                (try_begin),
                    (troop_get_slot, ":num_tasks", ":companion", 1000),
                    (gt, ":num_tasks", 0),
