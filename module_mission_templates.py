...
# ====================================================================================================================
# CUSTOM BOSS FIGHT TRIGGERS
# ====================================================================================================================
slot_agent_is_boss_guard = 401

boss_fight_triggers = [
    
    # 1. INITIALIZATION TRIGGER
    # Runs once at the beginning of the battle to clear state variables.
    (ti_before_mission_start, 0.0, 0.0, [], [
        (assign, "$g_boss_agent_id", -1),
        (assign, "$g_boss_fight_phase", 0),
        (assign, "$g_boss_spawn_90", 0),
        (assign, "$g_boss_spawn_75", 0),
        (assign, "$g_boss_spawn_50", 0),
        (assign, "$g_boss_spawn_25", 0),
        (assign, "$g_boss_spawn_count", 0),
        (assign, "$g_boss_last_hp", 0),
    ]),

    # 2. DETECT ENEMY BOSS ON SPAWN (Now runs periodically until found)
    # Detects if any active agent is an enemy Lord.
    (1.0, 0.0, 0.0, [
        (eq, "$g_boss_agent_id", -1),
        (get_player_agent_no, ":player_agent"),
        (gt, ":player_agent", -1),
        ...
        (try_for_agents, ":agent_no"),
            ...
            (agent_is_human, ":agent_no"),
            (agent_get_troop_id, ":troop_no", ":agent_no"),
            (is_between, ":troop_no", active_npcs_begin, active_npcs_end), # Is a Lord
            (agent_get_team, ":team", ":agent_no"),
            (neq, ":team", ":player_team"), # Boss is on the enemy team!
            (assign, "$g_boss_agent_id", ":agent_no"),
            (assign, "$g_boss_fight_phase", 0),
            (assign, ":boss_found", 1),
            (display_message, "@[BOSS ACTIVE] Face the enemy commander! Slay their army first!", 0xFFFF44),
        (try_end),
    ]),

    # 3. TRANSITION TO PHASE 1 (GUARDS PHASE) AFTER MAIN ARMY FALLS
    # Detects when there are no more non-boss enemy soldiers left, initializing the boss phase.
    (1.0, 0.0, 0.0, [
        (gt, "$g_boss_agent_id", -1),
        (agent_is_active, "$g_boss_agent_id"),
        (agent_is_alive, "$g_boss_agent_id"),
        (eq, "$g_boss_fight_phase", 0),
        (get_player_agent_no, ":player"),
        (gt, ":player", -1),
        (agent_is_active, ":player"),
        (store_mission_timer_a, ":timer"),
        (gt, ":timer", 5), # Wait at least 5 seconds for all initial AI and spawns
    ], [
        # Count all other living enemy human agents
        (assign, ":other_enemies", 0),
        (get_player_agent_no, ":player_agent"),
        (agent_get_team, ":player_team", ":player_agent"),
        (try_for_agents, ":cur_agent"),
            ...
            (neq, ":cur_agent", "$g_boss_agent_id"),
            (agent_get_team, ":cur_team", ":cur_agent"),
            (neq, ":cur_team", ":player_team"), # Is an enemy soldier
            (val_add, ":other_enemies", 1),
        (try_end),

        (try_begin),
            (eq, ":other_enemies", 0), # Only the boss remains!
            (assign, "$g_boss_fight_phase", 1),
            
            # Fade out player's army when event begins so it is a true duel
            (try_for_agents, ":ally_agent"),
                (agent_is_active, ":ally_agent"),
                (agent_is_alive, ":ally_agent"),
                (agent_is_human, ":ally_agent"),
                (neq, ":ally_agent", ":player_agent"),
                (agent_get_team, ":ally_team", ":ally_agent"),
                (eq, ":ally_team", ":player_team"),
                (agent_fade_out, ":ally_agent"),
            (try_end),

            (play_sound, "snd_tutorial_1"),
            (display_message, "@[BOSS PHASE 1] The enemy army has fallen! The Commander retreats and summons his personal Elite Guard!", 0xFF3333),
            
            # Identify the boss's faction to spawn corresponding elite guards
            (agent_get_troop_id, ":boss_troop", "$g_boss_agent_id"),
            (store_troop_faction, ":faction", ":boss_troop"),

            # Make boss MUCH faster and smarter at blocking by capping proficiencies!
            (troop_raise_proficiency_linear, ":boss_troop", 0, 300), # 1H
            (troop_raise_proficiency_linear, ":boss_troop", 1, 300), # 2H
            (troop_raise_proficiency_linear, ":boss_troop", 2, 300), # Polearm
            (troop_raise_attribute, ":boss_troop", 1, 10), # Agility buff for speed
            (agent_set_speed_modifier, "$g_boss_agent_id", 150),
            
            # Map Faction to corresponding elite guard troop type
            (assign, ":guard_troop", "trp_hired_blade"), # Default fallback
            (try_begin),
                (eq, ":faction", "fac_kingdom_1"), # Swadia
                (assign, ":guard_troop", "trp_swadian_knight"),
            (else_try),
                (eq, ":faction", "fac_kingdom_2"), # Vaegirs
                (assign, ":guard_troop", "trp_vaegir_knight"),
            (else_try),
                (eq, ":faction", "fac_kingdom_3"), # Khergits
                (assign, ":guard_troop", "trp_khergit_lancer"),
            (else_try),
                (eq, ":faction", "fac_kingdom_4"), # Nords
                (assign, ":guard_troop", "trp_nord_veteran"),
            (else_try),
                (eq, ":faction", "fac_kingdom_5"), # Rhodoks
                (assign, ":guard_troop", "trp_rhodok_sergeant"),
            (else_try),
                (eq, ":faction", "fac_kingdom_6"), # Sarranids
                (assign, ":guard_troop", "trp_sarranid_mamluke"),
            (try_end),
            
            # Spawn 5 guards in a defensive formation back of the boss
            (agent_get_position, pos1, "$g_boss_agent_id"),
            (agent_get_team, ":boss_team", "$g_boss_agent_id"),
            
            (try_for_range, ":i", 0, 5),
                (copy_position, pos2, pos1),
                (try_begin),
                    (eq, ":i", 0), (position_move_x, pos2, -250), (position_move_y, pos2, -400),
                (else_try),
                    (eq, ":i", 1), (position_move_x, pos2, 250), (position_move_y, pos2, -400),
                (else_try),
                    (eq, ":i", 2), (position_move_x, pos2, -150), (position_move_y, pos2, -600),
                (else_try),
                    (eq, ":i", 3), (position_move_x, pos2, 150), (position_move_y, pos2, -600),
                (else_try),
                    (eq, ":i", 4), (position_move_y, pos2, -800),
                (try_end),
                (set_spawn_position, pos2),
                (spawn_agent, ":guard_troop"),
                (assign, ":spawned", reg0),
                (agent_set_team, ":spawned", ":boss_team"),
                (agent_set_is_alarmed, ":spawned", 1),
                (agent_set_slot, ":spawned", slot_agent_is_boss_guard, 1),
            (try_end),
        (try_end),
    ]),

    # 4. ALLY CHECK AND RETREAT BEHAVIOR (APPLIES TO ALL PHASES)
    # If the boss has any living allies, he becomes immune and retreats from the player.
    # If he has no allies, he attacks!
    (0.5, 0.0, 0.0, [
        (gt, "$g_boss_agent_id", -1),
        (agent_is_active, "$g_boss_agent_id"),
        (agent_is_alive, "$g_boss_agent_id"),
        (lt, "$g_boss_fight_phase", 3),
        (get_player_agent_no, ":player"),
        (gt, ":player", -1),
        (agent_is_active, ":player"),
    ], [
        ...
        # Check if boss has any living allies remaining in the scene
        (assign, ":boss_allies_alive", 0),
        (try_for_agents, ":cur_agent"),
            (agent_is_active, ":cur_agent"),
            (agent_is_alive, ":cur_agent"),
            (agent_is_human, ":cur_agent"),
            (neq, ":cur_agent", "$g_boss_agent_id"),
            (agent_get_team, ":cur_team", ":cur_agent"),
            (neq, ":cur_team", ":player_team"), # Is an enemy of the player
            (val_add, ":boss_allies_alive", 1),
        (try_end),
        
        (try_begin),
            (gt, ":boss_allies_alive", 0),
            # Boss has allies => RETREAT & IMMUNE
            (agent_set_hit_points, "$g_boss_agent_id", 100, 0),
            
            (agent_get_position, pos10, ":player"),
            (agent_get_position, pos11, "$g_boss_agent_id"),
            (get_distance_between_positions, ":dist", pos10, pos11),
            
            (copy_position, pos12, pos11),
            (try_begin),
                (lt, ":dist", 1800), # If player is within 18m, move backward slightly to maintain safe distance
                (position_move_y, pos12, -800), # Move backward 8m
            (try_end),
            # ALWAYS be scripted when allies are alive, so he doesn't charge!
            (agent_set_scripted_destination, "$g_boss_agent_id", pos12, 1),
            (agent_force_rethink, "$g_boss_agent_id"),
            
            # Ensure phase reflects guard states
            (try_begin),
                (eq, "$g_boss_fight_phase", 2),
                (assign, "$g_boss_fight_phase", 1), # Revert to phase 1 (Guards Alive phase)
            (try_end),
        (else_try),
            # Boss has NO allies => ATTACK & VULNERABLE
            (agent_clear_scripted_mode, "$g_boss_agent_id"),
            (try_begin),
                (eq, "$g_boss_fight_phase", 1),
                (assign, "$g_boss_fight_phase", 2), # Progress to phase 2 (Vulnerable charge phase)
                (play_sound, "snd_tutorial_2"),
                (display_message, "@[BOSS PHASE 2] The Commander charges with fury!", 0xFF4444),
            (try_end),
        (try_end),
    ]),

    # 5. MONITOR BOSS STATUS (KILLED OR OUT)
    (1.0, 0.0, 0.0, [
        (gt, "$g_boss_agent_id", -1),
        (ge, "$g_boss_fight_phase", 1),
        (lt, "$g_boss_fight_phase", 3),
        (this_or_next|neg|agent_is_active, "$g_boss_agent_id"),
        (neg|agent_is_alive, "$g_boss_agent_id"),
    ], [
        (assign, "$g_boss_fight_phase", 3),
        (display_message, "@[BOSS DEFEATED] The Commander has been vanquished!", 0x33FF33),
    ]),

    # 7. PHASE 2 DYNAMIC HEALTH BOUNDARY REINFORCEMENTS
    # Triggers dynamic spawns of supportive guards as the boss's HP drops to higher thresholds.
    (0.5, 0.0, 0.0, [
        (eq, "$g_boss_fight_phase", 2),
        (agent_is_active, "$g_boss_agent_id"),
        (agent_is_alive, "$g_boss_agent_id"),
        ...
        (store_agent_hit_points, ":hp", "$g_boss_agent_id", 0), # Percentage HP (0-100)
        (agent_get_position, pos1, "$g_boss_agent_id"),
        (agent_get_team, ":boss_team", "$g_boss_agent_id"),

        (assign, ":trigger_spawn", 0),
        (try_begin),
            (lt, ":hp", 75),
            (eq, "$g_boss_spawn_75", 0),
            (assign, "$g_boss_spawn_75", 1),
            (assign, ":trigger_spawn", 1),
            (display_message, "@[75% Boss Health] The Commander is wounded and calls 3 elite guards to defend him!", 0xFFFF33),
        (else_try),
            (lt, ":hp", 50),
            (eq, "$g_boss_spawn_50", 0),
            (assign, "$g_boss_spawn_50", 1),
            (assign, ":trigger_spawn", 1),
            (display_message, "@[50% Boss Health] The Commander is in critical danger and summons his final 3 elite guards!", 0xFFFF33),
        (try_end),

        (try_begin),
            (eq, ":trigger_spawn", 1),
            (play_sound, "snd_tutorial_1"),
            (try_for_range, ":idx", 0, 3), # Max 3 soldiers each spawn
                (copy_position, pos2, pos1),
                (try_begin),
                    (eq, ":idx", 0), (position_move_x, pos2, -200), (position_move_y, pos2, -300),
                (else_try),
                    (eq, ":idx", 1), (position_move_x, pos2, 200), (position_move_y, pos2, -300),
                (else_try),
                    (eq, ":idx", 2), (position_move_y, pos2, -500),
                (try_end),
                (set_spawn_position, pos2),
                (spawn_agent, ":guard_troop"),
                (assign, ":spawned", reg0),
    ...
    # 8. BOSS DAMAGE REDUCTION, IMMUNITY BEFORE EVENT & HARDCORE DODGE
    (ti_on_agent_hit, 0.0, 0.0, [
        (store_trigger_param_1, ":receiver"),
        (eq, ":receiver", "$g_boss_agent_id"),
    ], [
        (store_trigger_param_3, ":damage"),
        
        (try_begin),
            (lt, "$g_boss_fight_phase", 1), # Event has not started yet! Immune to damage.
            (assign, ":damage", 0),
        (else_try),
            # Hardcore deflect: 42% chance to completely parry/block/dodge the attack
            (store_random_in_range, ":dodge_chance", 0, 100),
            (try_begin),
                (lt, ":dodge_chance", 42),
                (display_message, "@The Commander expertly blocks and parries your strike!", 0x44FF44),
                (assign, ":damage", 0),
            (else_try),
                (val_div, ":damage", 6), # Master Defense: Boss takes only 1/6th damage, giving him a massive health pool!
            (try_end),
            (agent_set_speed_modifier, "$g_boss_agent_id", 155), # Keep match blisteringly fast and challenging
        (try_end),
...
# --- HOTKEY 'T' TO OPEN AI CHAT (INDOORS) ---
# --- INDOOR UI OPENER ---
# Press '.' Indoors to prepare message
common_ai_chat_opener = (0, 0, 0, [(eq, "$g_open_ai_chat_flag", 1)], [
...
##          (agent_get_team, reg3, ":dead_agent_no"),
    ...
    ##diplomacy begin
    + dplmc_battle_mode_triggers + dplmc_horse_cull + boss_fight_triggers,  #SB : horse cull + custom boss battle
    ##diplomacy end
  ),
    ...
    [
      ...
      (1, 60, ti_once,
       [
         ...
         ##diplomacy begin
         (this_or_next|eq, "$g_dplmc_battle_continuation", 0),
         # Custom Boss Battle Check: only reward victory if there's no boss active, or boss is defeated
         (this_or_next|eq, "$g_boss_agent_id", -1),
         (eq, "$g_boss_fight_phase", 3),
         (neg|main_hero_fallen),
         ##diplomacy end
         (set_mission_result,1),
         (display_message,"str_msg_battle_won"),
