...
namespace CalradiaAiBridge {
    partial class Program {
        ...
        static async Task<string> GetCloudResponse( string text, Dictionary<string, object> data ) {
            ...
            string systemPrompt = string.Format("You are {0}, a {1} in the medieval world of Mount & Blade: Warband.{2} {3} Talk naturally with correct spacing after punctuation - NEVER run words together (like 'doorsteptrade' instead of 'doorstep. Trade'). Limit your response to 1-3 short sentences. Always reply in the same language that the user is speaking to you. If you like the player or they are respectful, append [RELATION_UP]. If they are insulting or hostile, append [RELATION_DOWN].", name, role, roleContext, relationStr);
            if (isCompanion) {
        ...
        static async Task<string> GetPlayer2ApiResponse( string text, Dictionary<string, object> data ) {
            ...
            string systemPrompt = string.Format("Roleplay as {0}, a {1} of the {2} in the world of Calradia.{3} {4} Respond strictly in character with a gritty medieval tone. Your response MUST be ONLY the spoken dialogue. Limit your response to 1-3 short sentences. Always reply in the same language that the user is speaking to you. If you like the player or they are respectful, append [RELATION_UP]. If they are insulting or hostile, append [RELATION_DOWN].", name, role, kingdom, roleContext, relationStr);
            bool isCompanion = role == "companion" || role.Contains("companion") || role.Contains("member");
            if (isCompanion) {
            ...
            try {
                ...
                if (res.IsSuccessStatusCode) {
                    Console.WriteLine("[DEBUG] HTTP Headers:");
                    ...
                    using (var reader = new System.IO.StreamReader(stream)) {
                        while (!reader.EndOfStream) {
                            ...
                            Console.WriteLine("[DEBUG] Stream Chunk: " + line);
                                ...
                                try {
                                    ...
                                    if (json != null && json.ContainsKey("choices")) {
                                        ...
                                        if (firstChoice != null && firstChoice.ContainsKey("message")) {
                                            ...
                                            } else if (msgObj != null && msgObj.ContainsKey("text") && msgObj["text"] != null) {
                                                textResponse += msgObj["text"].ToString();
                                            }
                                        }
                                        if (string.IsNullOrWhiteSpace(textResponse) && firstChoice != null && firstChoice.ContainsKey("text")) {
                                            textResponse += firstChoice["text"].ToString();
                                        }
                                        if (string.IsNullOrWhiteSpace(textResponse) && firstChoice != null && firstChoice.ContainsKey("response")) {
                                            textResponse += firstChoice["response"].ToString();
                                        }

                                        // Some APIs return text in a different root property if choices fails
                                        if (string.IsNullOrWhiteSpace(textResponse) && json.ContainsKey("text")) {
                                            textResponse = json["text"].ToString();
                                        }
                                        if (string.IsNullOrWhiteSpace(textResponse) && json.ContainsKey("response")) {
