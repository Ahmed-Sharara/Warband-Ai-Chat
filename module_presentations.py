...
# ==========================================
  # PRESENTATION 1: INDOOR SCENES
  # ==========================================

  # Presentation for black editbox
("ai_chat_input", 2, "mesh_load_window", [ # Flag 2 = Locks camera and pauses World Map time
    (ti_on_presentation_load, [
      (set_fixed_point_multiplier, 1000),
      
      # 1. THE LABEL
      (create_text_overlay, "$g_ai_label", "@TYPE MESSAGE AND PRESS ENTER:", 0),
      (position_set_x, pos1, 280), (position_set_y, pos1, 450), (overlay_set_position, "$g_ai_label", pos1),

      # 2. THE BLACK EDIT BOX 
      (create_simple_text_box_overlay, "$g_ai_input_box"),
      (position_set_x, pos1, 400), (position_set_y, pos1, 400), (overlay_set_position, "$g_ai_input_box", pos1),
    ...
    (ti_on_presentation_run, [
      ...
      (try_begin),
        ...
        # 1. Relation
        (str_store_string, s1, "@relation"),
        (call_script, "script_troop_get_player_relation", "$g_ai_talk_troop"),
        (assign, ":relation", reg0),
        (dict_set_int, "$ai_out", s1, ":relation"),
        ...
        (else_try),
            (str_store_string, s6, "@World Map"),
        (try_end),
        (dict_set_str, "$ai_out", s1, s6),

        # 3. Role
        (try_begin),
            (eq, "$g_ai_role_id", 1), 
            (try_begin),
               (is_between, ":f", "fac_kingdom_1", "fac_kingdoms_end"),
               (faction_get_slot, ":leader", ":f", slot_faction_leader),
               (eq, "$g_ai_talk_troop", ":leader"),
               (str_store_string, s7, "@King"),
            (else_try),
               (str_store_string, s7, "@Lord"),
            (try_end),
        (else_try),
            (eq, "$g_ai_role_id", 2), (str_store_string, s7, "@Guildmaster"),
        (else_try),
            (eq, "$g_ai_role_id", 3), (str_store_string, s7, "@Village Elder"),
        (else_try),
            (eq, "$g_ai_role_id", 4), (str_store_string, s7, "@Companion"),
        (else_try),
            (eq, "$g_ai_role_id", 5), (str_store_string, s7, "@Tavernkeeper"),
        (else_try),
            (eq, "$g_ai_role_id", 6), (str_store_string, s7, "@Merchant"),
        (else_try),
            (eq, "$g_ai_role_id", 7), (str_store_string, s7, "@Arena Master"),
        (else_try),
            (eq, "$g_ai_role_id", 8), (str_store_string, s7, "@Spouse"),
        (else_try),
            (eq, "$g_ai_role_id", 9), (str_store_string, s7, "@Peasant"),
        (else_try),
            (eq, "$g_ai_role_id", 10), (str_store_string, s7, "@Slaver"),
        (else_try),
            (eq, "$g_ai_role_id", 11), (str_store_string, s7, "@Traveler"),
        (else_try),
            (eq, "$g_ai_role_id", 12), (str_store_string, s7, "@Ransom Broker"),
        (else_try),
            (eq, "$g_ai_role_id", 13), (str_store_string, s7, "@Guard"),
        (else_try),
            (eq, "$g_ai_role_id", 14), (str_store_string, s7, "@Prison Guard"),
        (else_try),
            (eq, "$g_ai_role_id", 15), (str_store_string, s7, "@Lady"),
        (else_try),
            (str_store_string, s7, "@Commoner"),
        (try_end),
        (str_store_string, s1, "@role"), (dict_set_str, "$ai_out", s1, s7),

        # 4. Faction Leader (King)
        (str_store_string, s1, "@king"),
        (try_begin),
            (is_between, ":f", "fac_kingdom_1", "fac_kingdoms_end"),
            (faction_get_slot, ":leader", ":f", slot_faction_leader),
            (gt, ":leader", 0),
        ...
        (else_try),
            (str_store_string, s5, "@None"),
        (try_end),
        (dict_set_str, "$ai_out", s1, s5),
